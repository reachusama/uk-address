...
FLAT = {"FLAT", "FLT", "APARTMENT", "APPTS", "APPT", "APTS", "APT", "ROOM", "ANNEX", "ANNEXE", "UNIT", "BLOCK", "BLK"}
COMPANY = {"CIC", "CIO", "LLP", "LP", "LTD", "LIMITED", "CYF", "PLC", "CCC", "UNLTD", "ULTD"}
ROAD = {
    "ROAD", "RAOD", "RD", "DRIVE", "DR", "STREET", "STRT", "AVENUE", "AVENEU", "SQUARE",
...
Business = {
    "OFFICE", "HOSPITAL", "CARE", "CLUB", "BANK", "BAR", "UK", "SOCIETY", "PRISON", "HMP",
    "RC", "UWE", "UEA", "LSE", "KCL", "UCL", "UNI", "UNIV", "UNIVERSITY", "UNIVERISTY",
}
Locational = {
    "BASEMENT", "GROUND", "UPPER", "ABOVE", "TOP", "LOWER", "FLOOR", "HIGHER", "ATTIC",
    "LEFT", "RIGHT", "FRONT", "BACK", "REAR", "WHOLE", "PART", "SIDE",
}
Ordinal = {
...
non_county = {
    "OFFICE", "HOSPITAL", "CARE", "CLUB", "BANK", "BAR", "SOCIETY", "PRISON",
...
def readXML(xmlFile: str) -> Iterator[Tuple[str, List[Tuple[str, str]]]]:
    ...
    for component_string in component_string_list:
        ...
        raw_text = etree.tostring(sequence_xml, method="text", encoding="utf-8").decode("utf-8")
...
def tokenFeatures(token: str) -> Dict:
    ...
    features = {
        ...
        "length": ("d:" + str(len(token_clean)) if token_clean.isdigit() else "w:" + str(len(token_clean))),
...
def removeCounties(in_string: str) -> str:
    ...
    look_behind = r"(?<!\b{0})({1})".format(r")(?<!\b".join(c_except), "|".join(map(re.escape, counties)))
